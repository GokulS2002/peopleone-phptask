...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || !validate_csrf_token($_POST['csrf_token'])) {
        ...
        // Email validation and domain check for disposable emails
        $disposableDomains = ['mailinator.com', 'tempmail.com', '10minutemail.com', 'fakeinbox.com'];
        $emailDomain = substr(strrchr($email, "@"), 1);

        // Validate required fields and specific checks
        if (empty($username)) {
...
document.getElementById('registerForm').addEventListener('submit', function (event) {
    ...
    // Check password field for length and character requirements
